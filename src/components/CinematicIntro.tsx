...
// First 25 cards per column appear in the first ~1s; rest are off-screen until 600ms
...
export const CinematicIntro = ({ onComplete }: CinematicIntroProps) => {
  ...
  useEffect(() => {
    ...
    // Remaining cards are off-screen at 600ms — swap placeholders invisibly
    const batchTimer = setTimeout(() => setShowAll(true), BATCH_DELAY);
    return () => {
      ...
      <div className="absolute inset-0 flex gap-3 px-3">
        {columns.map((games, ci) => (
          <div key={ci} className="flex-1 overflow-hidden">
            ...
            >
              {games.map((game, idx) => {
                const renderFull = idx < INITIAL_BATCH || showAll;
                return (
                  <div key={idx} style={{ height: CARD_HEIGHT, flexShrink: 0 }}>
                    {renderFull ? (
                      <div
                        className={`relative w-full h-full rounded-2xl overflow-hidden border border-black/5 shadow-sm ${
                          !game.image ? `bg-gradient-to-br ${getFallbackGradient(game.genre)}` : 'bg-zinc-900'
                        }`}
                      >
                        {game.image && (
                          <img
                        ...
                        <div className="relative h-full z-10 p-3 flex flex-col justify-between">
                          <div className="flex justify-between items-start">
                            <span className="text-[8px] font-black text-white bg-white/20 backdrop-blur-sm px-1.5 py-0.5 rounded tracking-widest uppercase">
                              {game.genre}
                            </span>
                      ...
                      // 아직 화면 밖에 있는 카드 — 공간만 차지하는 경량 placeholder
                      <div className="w-full h-full rounded-2xl bg-zinc-200/50" />
                    )}
                  </div>
                );
              })}
